...
Once access is granted, download the required MIMIC-IV version (v2.2) and place the raw files in the directory specified by `RawDataPath` in the configuration file.
...
| Parameter | Description |
|------------|------------|
| `Outliers_management` | `remove`, `impute`, `No_outlier_detection` |
...
| Parameter | Description |
|------------|------------|
| `Missing_values_management` | `mean`, `median`, `No_imputation` |
...
This pipeline is an adaptation of an already existing work.

If you use this pipeline, cite the original paper:
