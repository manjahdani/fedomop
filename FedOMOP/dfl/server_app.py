...
def main(grid: Grid, context: Context) -> None:
    ...
    if server_strategy in ["SPWF"]:
        strategy = Router(fraction_evaluate=fraction_evaluate,
                          min_available_nodes=2, 
                          evaluate_metrics_aggr_fn=custom_aggregate_metricrecords)
    elif server_strategy in ["SOLO"]:
        strategy = Orchestrator(fraction_evaluate=fraction_evaluate,
                                min_available_nodes=2,
                                evaluate_metrics_aggr_fn=custom_aggregate_metricrecords)
    else:
